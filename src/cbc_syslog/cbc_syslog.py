...
def main(args):
    ...
    # Store Forward.  Attempt to send messages that have been saved due
    # to a failure to reach the destination
...
def run_main():
    ...
    argparser.add_argument(
        '--config-file', '-c', help="Absolute path to configuration file")
        ...
        for process in psutil.process_iter():
            try:
                if process.name() == 'cbc-syslog.pid':
                    logger.error(
                        "An instance of cbc syslog is already running")
                    sys.exit(0)
